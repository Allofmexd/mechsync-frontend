...
function LinesTable({ title, lines, type }) {
  return <section className="revision-detail-card"><header><h2>{title}</h2><span>{lines.length}</span></header>{lines.length === 0 ? <p className="revision-empty">Sin líneas snapshot.</p> : <div className="revision-table-wrap"><table className="revision-table"><thead><tr><th>#</th><th>Nombre snapshot</th><th>Descripción</th><th>Cantidad</th><th>Precio</th><th>Subtotal</th><th>Notas</th></tr></thead><tbody>{lines.map((line) => <tr key={line.id ?? line.lineNumber}><td>{line.lineNumber}</td><td><strong>{line.nameSnapshot}</strong>{type === 'part' && line.partNumberSnapshot && <small>{line.partNumberSnapshot}</small>}</td><td>{line.descriptionSnapshot || 'Sin descripción'}</td><td>{line.quantity}</td><td>{formatRevisionMoney(line.unitPrice)}</td><td>{formatRevisionMoney(line.lineSubtotal)}</td><td>{line.notes || 'Sin notas'}</td></tr>)}</tbody></table></div>}</section>;
}

function formatIvaRate(value) {
  const rate = Number(value);
  return Number.isFinite(rate)
    ? new Intl.NumberFormat('es-MX', { style: 'percent', maximumFractionDigits: 4 }).format(rate)
    : 'Dato no disponible';
}
  ...
  return <section className="revision-detail-page">
    ...
    <div className="revision-detail-grid">
      <div className="revision-detail-main">
        <section className="revision-detail-card"><header><h2>Planeación y observaciones</h2></header><dl className="revision-detail-list"><div><dt>Técnico</dt><dd>#{revision.technicianId}</dd></div><div><dt>Inicio estimado</dt><dd>{formatRevisionDate(revision.estimatedStartDate)}</dd></div><div><dt>Entrega estimada</dt><dd>{formatRevisionDate(revision.estimatedDeliveryDate)}</dd></div><div><dt>Horas estimadas</dt><dd>{revision.estimatedHours ?? 'Dato no disponible'}</dd></div><div><dt>Observaciones técnicas</dt><dd>{revision.technicalObservations || 'Sin observaciones'}</dd></div><div><dt>Notas para el cliente</dt><dd>{revision.customerNotes || 'Sin notas para el cliente'}</dd></div><div><dt>Tratamiento fiscal</dt><dd>{revision.taxTreatmentNotes || 'Sin notas fiscales'}</dd></div><div><dt>Motivo del cambio</dt><dd>{revision.changeReason || 'Primera revisión o motivo no requerido'}</dd></div></dl></section>
      ...
      <aside className="revision-detail-side">
        <section className="revision-detail-card"><header><h2>Importes</h2></header><dl><div><dt>Moneda</dt><dd>{revision.currency || 'MXN'}</dd></div><div><dt>Subtotal</dt><dd>{formatRevisionMoney(revision.subtotalAmount, revision.currency)}</dd></div><div><dt>IVA aplicado</dt><dd>{revision.applyIva ? `Sí · ${formatIvaRate(revision.ivaRate)}` : 'No'}</dd></div><div><dt>IVA</dt><dd>{formatRevisionMoney(revision.ivaAmount, revision.currency)}</dd></div><div className="revision-total"><dt>Total</dt><dd>{formatRevisionMoney(revision.totalAmount, revision.currency)}</dd></div></dl><small>Importes devueltos por la API.</small></section>
        ...
        <section className="revision-detail-card"><header><h2>Auditoría</h2></header><dl><div><dt>Creada por</dt><dd>{revision.createdByUserId ? `Usuario #${revision.createdByUserId}` : 'Dato no disponible'}</dd></div><div><dt>Creada</dt><dd>{formatRevisionDate(revision.createdAt)}</dd></div><div><dt>Aprobada por</dt><dd>{revision.approvedByUserId ? `Usuario #${revision.approvedByUserId}` : 'Sin aprobación'}</dd></div><div><dt>Aprobada</dt><dd>{formatRevisionDate(revision.approvedAt)}</dd></div><div><dt>Actualizada</dt><dd>{formatRevisionDate(revision.updatedAt)}</dd></div><div><dt>Versión de bloqueo</dt><dd>{revision.lockVersion}</dd></div></dl></section>
        <button className="admin-button admin-button--secondary" type="button" disabled title="La cotización no cuenta con endpoint PDF en esta versión">PDF de cotización no disponible</button>
      </aside>
    </div>
  </section>;
}
